...
void main()
{
 ...
 #else
    vec3 lightDir = normalize(-u_LightProps.Dir);
 #endif
    ...
    // Attenuation
    float dist = length(u_LightProps.Pos - v_FragmentPos);
    float attenuation = 1.0 / (u_LightProps.Attenuation0 + u_LightProps.Attenuation1 * dist + u_LightProps.Attenuation2 * (dist * dist));
...
void main()
{
    ...
    vec3 lightDir = normalize(u_LightProps.Pos - v_FragmentPos);
    float theta = dot(lightDir, normalize(-u_LightProps.Dir)); 
    
    if (theta > u_LightProps.Falloff)
    {
        // Ambient
        vec3 ambient = u_Material.Ambient * u_LightProps.Ambient;
    ...
    else
    {
        ...
        if (u_Material.bUseTexture == 1)
        {
            outputColor = texture(u_Texture, v_Uv) * vec4((ambient + emissive), 1.0);
        ...
        {
            outputColor = vec4((ambient + emissive), 1.0);
        }
